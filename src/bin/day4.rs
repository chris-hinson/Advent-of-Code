...
impl Board {
    ...
    //take a value and if the board contains it, mark it
    pub fn mark(&mut self, value: i32) {
        for (i, line) in self.board.iter_mut().enumerate() {
            for (j, col) in line.iter_mut().enumerate() {
                if col.0 == value {
    ...
    pub fn done(&mut self) -> bool {
        ...
        self.done = row_found || col_found;
        return row_found || col_found;
    }

    pub fn tabulate_score(&mut self, last_num: i32) {
        ...
        self.score = unmarked_total * last_num;
...
fn main() {
    ...
    let mut bingo_vec = bingo_string
        .split(",")
    ...
    for (_board_num, board_contents) in lines.chunks(6).enumerate() {
        ...
        for (_line_num, line) in board_contents.iter().filter(|x| !x.is_empty()).enumerate() {
    ...
    //part2
    //iterate over called numbers
    for (i, number) in bingo_vec.iter_mut().enumerate() {
        //iterate over all the boards and mark the current number
        //only mark boards which have not already been finished
        boards
            .iter_mut()
            .filter(|y| !y.done)
            .for_each(|x| x.mark(*number));

        //iterate over all boards and if one is done, mark its score
        boards.iter_mut().for_each(|x| {
            //only
            if !x.done {
                if x.done() {
                    x.tabulate_score(*number);
                    x.order = i as i32;
                }
            }
        });
    ...
    let last = boards
        .iter_mut()
        .filter(|y| y.score != 0)
        .max_by_key(|x| x.order)
        .unwrap();
    println!("last board is \n{} \n with a score of {}", last, last.score);
}
